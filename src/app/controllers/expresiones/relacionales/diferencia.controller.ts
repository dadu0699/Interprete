...
export class Diferencia extends Nodo {
  ...
  public ejecutar(tabla: Tabla, arbol: Arbol) {
    ...
    if (this.opIzquierdo.tipo == Tipo.STRING && this.opDerecho.tipo == Tipo.STRING) {
      ...
      const excepcion: Excepcion = new Excepcion('Semántico',
        `Error de tipos en la operación relacional 'diferencia' se está tratando de operar ${this.opIzquierdo.tipo} y ${this.opDerecho.tipo}`,
