...
export class Division extends Nodo {
    ...
    if (this.opIzquierdo.tipo == Tipo.INTEGER && this.opDerecho.tipo == Tipo.INTEGER) {
      ...
      if (resIzquierdo == 0) {
        const excepcion: Excepcion = new Excepcion('Semantico',
          'Error aritmetico, la división con cero no esta permitida',
      ...
      const excepcion: Excepcion = new Excepcion('Semántico',
        `Error de tipos en la división se está tratando de operar ${this.opIzquierdo.tipo} y ${this.opDerecho.tipo}`,
        this.linea, this.columna);
      arbol.excepciones.push(excepcion);
      arbol.consola.push(excepcion.toString());
      return excepcion;
